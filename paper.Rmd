...
ggsave("mean_clusters_sBZ.png", height = 10, width =  20, units = "cm", dpi = 300  )
...
points <-  c(1,1+which(diff(analysis_by_radius$max_normalized)!=0))
...
sBZ1points<- as.data.frame(cbind(analysis_by_radius$radius[points], analysis_by_radius$max_normalized[points]))

sBZ_labels <- subset(sBZ1points, sBZ1points$V1 == 16 | sBZ1points$V1 == 65 | sBZ1points$V1 == 88 | sBZ1points$V1 == 144 | sBZ1points$V1 == 374) 

ggplot()+
  geom_line(data = analysis_by_radius, aes(x = radius, y = max_normalized))+
  geom_point(data = sBZ1points, aes(x=sBZ1points$V1, y=sBZ1points$V2))+
   geom_label_repel(aes(x=sBZ_labels$V1, y=sBZ_labels$V2, label = sBZ_labels$V1),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
                  segment.color = "grey50",
                  color = "grey50")+
...
If the dataset is forced to split into two groups the step at 88m radius turns out to be the turning point if the maximum normalized cluster size is used, and 86m if the mean cluster size is calculated.
...
# same late bronze age
turn_sBZ <- e.divisive(as.matrix(diff(analysis_by_radius$max_normalized)), k = 1, min.size = nrow(analysis_by_radius)/10) 

sBZ_turn_r <-analysis_by_radius$radius[turn_sBZ$estimates]
...
The late bronze age features are mostly concentrated in the western half and on the eastern-most part of the transsect. As can be seen by the rapidly rising graphs describing the maximum cluster size as well as the mean cluster size in relation to the percolation radius, the features cluster closely by each other. First steps in the distribution can be seen at 


The largest excavation site of the late bronze age in middle Germany is to be found in Zwenkau(-Eythra), which was excavated in advance of coal extraction proceses. The excavation area in Zwenkau is at the largest dimensions 1100 x 825 m [@huth_1998, 194].
