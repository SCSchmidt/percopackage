...
##' input data needed, which consists of a dataframe with the PLCINDEX, EASTING, NORTHING - header. A distance table with the headers 'ID1','ID2','d12' SM: may be used if a computation of the euclidean distance between points is not required. 
##' input of radius values needed 
#SM: NOTE that if the distance table option is used the Spatial dataframe MUST ALSO BE PROVIDED for the mapping!
#SM: both file types need to be comma delimited!
...
# SM:  The coordinate data is still required, even if distance table is provided!
# SM: generate this as the data.csv file for the mapping program
...
if (is.null(distance_table)) {
	...
	print(t2)
	  ...
	  #SM: Write file with list of PlcIndex used
	  file_name <- paste(file.path(path_working,"PlcIndex.csv"))
	  write.table(data_unique$PlcIndex, file_name, row.names=FALSE, col.names="PlcIndex")

	  #SM: save distance_table as nodes_list_d.text so it corresponds to work flow
	  file_name <- paste(file.path(path_working,"nodes_list_d.txt"))
