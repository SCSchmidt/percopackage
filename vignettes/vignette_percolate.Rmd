...
**graph_edgelist()** – creates a graph from the sub-matrix, with directed=TRUE to count the edges only once.
...
The example shape file is an outline of Britain, taken from: 
...
Both datasets will be loaded by
...
With these parameters defined, call the percolate function. Note that the second parameter defaults to "points". Any other value will interpret the data as a distance table (see notes for details). This function call will use the point data set to compute a distance table, then generate a cluster table for the radius and step values given.

```{r percolate function points}
percolate(data,,limit,radius_unit,upper_radius,lower_radius,step_value)
...
```{r}
percolate(data,distance_table,limit,radius_unit,upper_radius,lower_radius,step_value)
```


Now for mapping the clusters. 

At first the parameters need to be set. For plotting the maps with an outline or background, a shape file is needed. In the data set, it is included with the name "shape".
...
mapClusters(shape,map_name,source_file_name,dpi)
```
